...
interface ColDef {
  ...
  sub: string;
  width: number;
  group: 'r' | 'ntat' | 'ntr' | 'prod' | 'atr' | 'u' | 'uenc' | 'msg' | 'v' | 'venc';
}
  ...
  return (
    <div>
      ...
      <div className="mt-2 flex flex-wrap gap-4 text-xs font-mono px-1">
        <span className="text-amber-300">u = Aᵀr+e1: 2×512B = <b>1024B</b> (16-bit)</span>
        <span className="text-cyan-300">enc₁₂(u): 2×384B = <b>768B</b></span>
        <span className="text-orange-300">v = tᵀr+e2+m: 1×512B = <b>512B</b> (16-bit)</span>
        <span className="text-sky-300">enc₁₂(v): 1×384B = <b>384B</b></span>
        <span className="text-green-300">ciphertext = enc(u)+enc(v) = 768+384 = <b>1152B</b></span>
      </div>
    </div>
  );
}
